...
import json
import datetime
from myfunc import get_filename, get_file_name_ext , Timer, call
import time
import webbrowser, platform, subprocess
t = Timer()

#test github
version = "V09_20210603"
...
#version = "V07_20210531" support output template from ECT tool, support CIQ excel input from ECT tool
#version = "V08_20210602": add some plan menu, add help menu, with have link to download
#version = "V09_20210603": ongoing develop to support window, add some exception to exit profile loading when file path does not exist
#show profile sort by latest modify for easy looking
#test successful create profile, package, backup profile & package for window
#do not save package if package_name is empty
...
def ls_name(folder_path, e=""):
	...
	if e == "":
		...
		for filepath in sorted_by_mtime_ascending:
			filenames.append(get_filename(filepath))
		return filenames
def open_url_by_webbrowser(url):
...
def save_profile_press():
	...
	if package_name != "":
		...
		with open(package_filepath, 'w') as outfile:
			json.dump(data2, outfile)
		text_var_status.set(text_var_status.get()+" | Package to: "+ get_filename(package_filename))
		#text_var_status.set("Save profile to "+ profile_filename)
		#####################
	...
	profile_files = ls_name(os.path.join(home_path,'profile'), e = 'profile_package')  #filter child folder profile_package
	...
	#profile_files = ls_name(os.path.join(home_path,'profile'))
	profile_package_files = ls_name(os.path.join(home_path,'profile','profile_package')) #no filter folder
	new_choices = profile_package_files
	for choice in new_choices:
		profile_package_select['menu'].add_command(label=choice, command=tk._setit(om_profile_package_var, choice))
	########
...
def loadprofile():
	...
	profile_filepath = os.path.join(home_path,"profile",profile_filename_selected)
	loadprofile_procedure(profile_filepath)

	text_var_status.set("FINISHED PROFILE LOADING" + profile_filename_selected)
	print_to_textbox("FINISHED PROFILE LOADING " + profile_filename_selected)
	#create_button.configure(bg="green2")
...
def show_profile_package_to_textbox():
	...
	with open(os.path.join(home_path,"profile","profile_package",profile_package_filename_selected)) as json_file:
		...
		print("all profile_filepath in package as below: ")
		print_to_textbox("all profile_filepath in package as below: ")
		for profile_filepath in profiles_filepath:
			#print full path, de biet duoc chay tren window hay linux
	...
	#update package name va entry package name, de backup cho tien, khoi manual fill
	from myfunc import get_file_name_ext
	name,ext = get_file_name_ext(profile_package_filename_selected)
	#text_var_packagename.set(get_filename(profile_package_filename_selected))
	text_var_packagename.set(name)
...
def about():
	...
	about_text_string = 'Coding by Hoang Le P'
	about_text_string = about_text_string + "\n" + version + "\n\n"
	about_text_string = about_text_string + "Link to download:"
	tk.Label(newWindow, text = about_text_string).pack()
	
	link  = 'https://github.com/ephucle/script_creater'
	label_sourcecodelink = tk.Label(newWindow, text = link, fg="blue")
	label_sourcecodelink.bind("<Button-1>", lambda e: open_url_by_webbrowser("https://github.com/ephucle/script_creater"))
	label_sourcecodelink.pack()
	#about_text = about_text+ "Version: " + version + "\n\n"
	
	

def open_file():
	print("open_file selected")
	#input_file_path_textfile = filedialog.askopenfilename(filetypes=[("Text files", ".txt .xml .mos")])
	input_file_path_textfile = filedialog.askopenfilename(filetypes=[("Text files", ".txt .xml .mos .*")])
	print("input_file_path_textfile",input_file_path_textfile)
	with open(input_file_path_textfile) as infile:
		print(infile.read())
		
		#using gedit to open text file for editor
		
		#proc = subprocess.Popen(['gedit', input_file_path_textfile])
		if platform.system() == "Linux":
			call('gedit '+input_file_path_textfile)
		elif platform.system() == "Windows":
			#subprocess.call(['notepad.exe', input_file_path_textfile])
			subprocess.call(['notepad++.exe', input_file_path_textfile])
		...
		#this command help to open file in multiple platform, linux or windown	
...
##row7
profile_files = ls_name(os.path.join(home_path,'profile'), e = 'profile_package')
...
print_to_textbox("home_path: "+ home_path)
print_to_textbox("current system: "+ platform.system())
